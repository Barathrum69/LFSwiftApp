...
- (void)defaultAndCreateView {
    ...
    if (_equalSize) {
        ...
        if (titlesScroll) {
            titlesScroll.width = kScreenWidth;
        }
        
        if (slideScroll) {
            slideScroll.width = kScreenWidth;
        }
    }
...
- (void)changeSelectIndexPage:(NSInteger)index {
    {
        ...
        [UIView animateWithDuration:animation_time animations:^{
            if (self->_headStyle != SegmentHeadStyleSlide) {
                [before_btn setTitleColor:self->_deSelectColor forState:UIControlStateNormal];
                [select_btn setTitleColor:self->_selectColor forState:UIControlStateNormal];
            }
            
            if (self->_fontScale) {
                before_btn.titleLabel.font = [UIFont systemFontOfSize:self->_fontSize];
                select_btn.titleLabel.font = [UIFont systemFontOfSize:self->_fontSize*self->_fontScale];
            }
            
            if (self->lineView) {
                self->lineView.width = select_btn.width*self->_lineScale;
                self->lineView.center = CGPointMake(select_btn.center.x, self->lineView.center.y);
            }
            
            if (self->arrow_layer) {
                self->arrow_layer.position = CGPointMake(self->lineView.width/2, self->lineView.height/2);
            }
            
            if (self->slideView) {
                //slide位置变化
                self->slideView.width = select_btn.width*self->_slideScale;
                self->slideView.center = CGPointMake(select_btn.center.x, self->slideView.center.y);
                //偏移
                CGRect convertRect = [self->slideView convertRect:titlesScroll.frame fromView:titlesScroll];
