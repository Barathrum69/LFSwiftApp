...
- (void)commonInit {
    ...
    [self setGradientLayer:self.selectionIndicatorStripLayer];
    
    self.selectionIndicatorStripLayer.cornerRadius = 2;
...
- (void)setSectionTitles:(NSArray *)sectionTitles {
    ...
    [self setNeedsLayout];
    [self setNeedsDisplay];//这里是个大坑 不在ViewDidLoad赋值就他妈刷新不出来 必须重新调用一下setNeedsDisplay 哭哭.
}
...
//            if ([titleString isEqualToString:@"关注"]) {
//                self.remindLayer =  [UILabel new];
//                self.remindLayer.frame = CGRectMake(rect.origin.x+rect.size.width, kWidth(3), kWidth(13), kWidth(13));
//                self.remindLayer.backgroundColor = [UIColor redColor];
//                self.remindLayer.cornerRadius  = kWidth(13)/2;
//                [self.scrollView addSubview:self.remindLayer];
//
//                self.remindText = [UILabel new];
//                self.remindText.frame = CGRectMake(rect.origin.x+rect.size.width, kWidth(3), kWidth(13), kWidth(13));
//                self.remindText.text = @"99";
//                self.remindText.font = [UIFont systemFontOfSize:kWidth(9)];
//                self.remindText.textColor = [UIColor whiteColor];
//                self.remindText.textAlignment = NSTextAlignmentCenter;
////                self.remindText.truncationMode = kCATruncationEnd;
////                self.remindText.contentsScale = [[UIScreen mainScreen] scale];
//                [self.scrollView addSubview:self.remindText];
                
//                NSLog(@"%d",self.remindLayer.)
//                os_release(self.remindText);
//                os_release(self.remindLayer);
                
//                CFRelease(self.remindText);
//                CFRelease(self.remindLayer);
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
//            widthToEndOfSelectedSegment = kWidth(10);
...
//            return CGRectMake(x + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, sectionWidth - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight);
            return CGRectMake((sectionWidth-kWidth(10))/2+x + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, kWidth(10), self.selectionIndicatorHeight);
...
//            self.remindLayer.actions    = nil;
//            self.remindText.actions     = nil;
            ...
            self.selectionIndicatorBoxLayer.frame = [self frameForFillerSelectionIndicator];
//            self.remindLayer.actions = newActions;
//            self.remindLayer.frame = CGRectMake(kScreenWidth*2, 0, 0, 0);
//            self.remindText.actions = newActions;
//            self.remindText .frame = self.remindLayer.frame ;
            if (notify)
                [self notifyForSegmentChangeToIndex:index];
        }
    }
...
//设置渐变色滑块
- (void)setGradientLayer:(CALayer *)superLayer
{
    CAGradientLayer *gradientLayer = [CAGradientLayer layer];
    gradientLayer.colors = @[(__bridge id)[UIColor colorWithRed:255/255.0 green:120/255.0 blue:53/255.0 alpha:1.0].CGColor, (__bridge id)[[UIColor colorWithRed:240/255.0 green:146/255.0 blue:70/255.0 alpha:1.0] colorWithAlphaComponent:0.8].CGColor];
    gradientLayer.locations = @[@0.4, @1.0];
    gradientLayer.startPoint = CGPointMake(0, 0);
    gradientLayer.endPoint = CGPointMake(1.0, 0);
    gradientLayer.frame = CGRectMake(0, 0, 10, 4);
...
//        [_remindText removeFromSuperview];
//        [_remindLayer removeFromSuperview];
//        self.remindText = nil;
//        self.remindLayer = nil;
//        [self setNeedsDisplay];
//        NSMutableDictionary *newActions = [[NSMutableDictionary alloc] initWithObjectsAndKeys:[NSNull null], @"position", [NSNull null], @"bounds", nil];
...
//        [self.scrollView.layer needsDisplay];
//    }
