...
//        [self addSubview:tagLabel];
        ...
        UIImageView *iconImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 46, 20)];
        iconImgView.image = [UIImage imageNamed:@"home_newsIcon"];
        ...
        if (titles) {
            ...
            self.index = 1;
            self.firstBtn = [self btnframe:CGRectMake(CGRectGetMaxX(iconImgView.frame) + 6, 0, [UIScreen mainScreen].bounds.size.width - 2 * 30, self.bounds.size.height)  titleColor:_titleColor action:@selector(clickBtn:)];
