...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _underlineHeight = 4;
        _cellSpacing = 18;
        _leftAndRightMargin = 19;
        self.backgroundColor = [UIColor colorWithRed:245/255.0 green:245/255.0 blue:245/255.0 alpha:1.0];
        self.titleNormalColor = [UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0];
        self.titleSelectedColor = [UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0];
        self.titleNomalFont = [UIFont systemFontOfSize:15];
        self.titleSelectedFont = [UIFont boldSystemFontOfSize:19];
        [self setupSubViews];
//        self.underline.backgroundColor = [UIColor colorWithRed:255/255.0 green:120/255.0 blue:53/255.0 alpha:1.0];
    }
    return self;
}
...
- (void)setupSubViews {
    ...
    [self.underline mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.height - self.underlineHeight - self.bottomMargin - HG_ONE_PIXEL);
...
- (void)setupMoveLineDefaultLocation {
    ...
    if (self.isFullScreen) {
        cellWidth = (self.frame.size.width - 2 * self.leftAndRightMargin - self.cellSpacing * (self.titles.count - 1)) / self.titles.count;
    }
    [self.underline mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(self.underlineWidth);
        make.left.mas_equalTo(self.leftAndRightMargin + (cellWidth-self.underlineWidth)/2.0);
    }];
}
...
- (void)updateMoveLineLocation {
    ...
    [UIView animateWithDuration:0.15 animations:^{
        [self.underline mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.height - self.underlineHeight - self.bottomMargin - HG_ONE_PIXEL);
            make.height.mas_equalTo(self.underlineHeight);
//            make.centerX.mas_equalTo(cell.titleLabel);
            make.width.mas_equalTo(self.underlineWidth);
            make.centerX.mas_equalTo(cell.titleLabel);
        }];
        [self.collectionView setNeedsLayout];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isFullScreen) {
        itemWidth = (self.frame.size.width - 2 * self.leftAndRightMargin - self.cellSpacing * (self.titles.count - 1)) / self.titles.count;
    }
...
- (UIView *)underline {
    if (!_underline) {
        ...
        _underline.layer.cornerRadius = _underlineHeight/2.0;
        ...
        [_underline.layer insertSublayer:gradientLayer atIndex:0];
    }
    return _underline;
}
