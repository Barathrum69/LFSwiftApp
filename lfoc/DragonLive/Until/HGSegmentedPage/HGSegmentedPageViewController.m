...
#define kHGWidth self.view.frame.size.width
...
- (void)viewDidLoad {
    ...
    [self.pageViewControllers enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [controller.view mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(idx * kHGWidth);
    ...
    weakSelf.categoryView.selectedItemHelper = ^(NSUInteger index) {
        ...
        [strongSelf.scrollView setContentOffset:CGPointMake(index * kHGWidth, 0) animated:NO];
...
{
    self.selectedIndex = index;
    [self.scrollView setContentOffset:CGPointMake(index * kHGWidth, 0) animated:NO];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    NSUInteger index = (NSUInteger)(self.scrollView.contentOffset.x / kHGWidth);
    [self.categoryView changeItemWithTargetIndex:index];
    self.currentPageViewController = self.pageViewControllers[index];
    self.selectedIndex = index;
    if (self.delegate && [self.delegate respondsToSelector:@selector(segmentedPageViewControllerDidEndDeceleratingWithPageIndex:)]) {
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        _scrollView.contentSize = CGSizeMake(kHGWidth * self.pageViewControllers.count, 0);
