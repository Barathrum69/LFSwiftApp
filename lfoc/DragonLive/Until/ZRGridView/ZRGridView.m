...
- (void)setupViews{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height) collectionViewLayout:self.gridViewLayoutAndStyle];
    ...
    _collectionView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        if (weakself.headerBlock) {
            weakself.page = 1;
            weakself.headerBlock(weakself.page);
        }
    }];
    _collectionView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        if (weakself.footerBlock) {
            weakself.page ++;
            weakself.footerBlock(weakself.page);
        }
    }];
