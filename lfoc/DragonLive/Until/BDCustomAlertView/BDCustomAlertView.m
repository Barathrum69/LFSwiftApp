...
- (void)showAlertWithTitle : (NSString *)title message : (NSString *)message cancelButtonTitle : (NSString *)cancelButtonTitle successButtonTitle : (NSString *)successButtonTitle withSuccessBlock:(void (^)())successBlock cancelBlock:(void (^)())cancelBlock
{
    ...
    if (cancelButtonTitle.length > 0) {
        self.cancelButton.frame = CGRectMake((self.alertView.width-kWidth(122*2))/3, 0, kWidth(122), kWidth(35));
        self.cancelButton.layer.cornerRadius = 5;
        self.cancelButton.layer.borderColor = [UIColor colorFromHexString:@"777777"].CGColor;
        self.cancelButton.layer.borderWidth = 1;
        [self.cancelButton setTitle:cancelButtonTitle forState:UIControlStateNormal];
        [self.cancelButton setTitleColor:[UIColor colorFromHexString:@"475F7B"] forState:UIControlStateNormal];

        [self.dismissButton setTitle:cancelButtonTitle forState:UIControlStateNormal];
    }
    if (successButtonTitle.length > 0) {
        self.okButton.frame = CGRectMake((self.alertView.width-kWidth(122*2))/3*2+kWidth(122), 0, kWidth(122), kWidth(35));
        self.okButton.layer.cornerRadius = 5;
        self.okButton.layer.borderColor = SelectedBtnColor.CGColor;
        self.okButton.layer.borderWidth = 1;
        self.okButton.backgroundColor = SelectedBtnColor;
