...
CGFloat const kTopMargin = 15.0;//contentView内的上边距
CGFloat const kBottomMargin = 15.0;//contentView内的下边距
CGFloat const kHorizonMargin = 18.0;//contentView内的水平边距
CGFloat const kHorizonPadding = 10.0;//button水平方向间距
CGFloat const kVerticalPadding = 10.0;//button垂直方向间距
CGFloat const kTitleMessagePadding = 15;//title和message垂直方向间距
CGFloat const kMessageButtonsPadding = 20.0;//buttons与message（或title）垂直方向间距

CGFloat const kButtonHeight = 33.0;//button的高度
CGFloat const kContentHorizonMargin = 65;//contentView距离屏幕两边的距离
...
- (void)layoutSubviews {
    ...
    self.lineView.frame = CGRectMake(0, yPos + kVerticalPadding, self.width - 2*kContentHorizonMargin, 0.6);
...
- (UIButton *)otherButtonWithTitle:(NSString *)title {
    return [self generalButtonWithTitle:title
                                   font:[UIFont systemFontOfSize:15.0]
                             titleColor:[UIColor whiteColor]
                        backgroundColor:[UIColor colorWithRed:246/255.0 green:124/255.0 blue:55/255.0 alpha:1.0]
                     highlightedBgColor:nil
                               isCancel:NO];
}
...
- (UIButton *)generalButtonWithTitle:(NSString *)title
    ...
    if (isCancel) {
        [button setTitleColor:[UIColor colorWithRed:246/255.0 green:124/255.0 blue:55/255.0 alpha:1.0] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageWithColor:[UIColor whiteColor]] forState:UIControlStateNormal];
        button.layer.borderWidth = 1.0;
        button.layer.borderColor = [UIColor colorWithRed:246/255.0 green:124/255.0 blue:55/255.0 alpha:1.0].CGColor;
    }
    [button addTarget:self action:@selector(buttonsClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIFont *)titleFont {
    if (!_titleFont) {
        _titleFont = [UIFont systemFontOfSize:16.0];//设置标题默认字体
    }
    return _titleFont;
}
...
- (UIColor *)titleColor {
    if (!_titleColor) {
        _titleColor = [UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0];//设置标题默认颜色
    }
    return _titleColor;
}
...
- (UIView *)contentView {
    if (!_contentView) {
        ...
        _contentView.layer.cornerRadius = 13.0;
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = [UIColor colorWithRed:238/255.0 green:238/255.0 blue:238/255.0 alpha:0.8];
    }
    return _lineView;
}
