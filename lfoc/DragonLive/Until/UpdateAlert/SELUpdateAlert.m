...
{
    NSString *_url;//跳转的url
    BOOL _isShowCancel; //是否显示关闭按钮
}
 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Descriptions:(NSArray *)descriptions  isShowCancel:(BOOL)isShowCancel url:(NSString *)url;
{
    ...
    NSLog(@"====%@",description);
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description isShowCancel:isShowCancel url:url];
 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Description:(NSString *)description isShowCancel:(BOOL)isShowCancel url:(NSString *)url;
{
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description isShowCancel:isShowCancel url:url];
    [[UIApplication sharedApplication].delegate.window addSubview:updateAlert];
}

- (instancetype)initVersion:(NSString *)version description:(NSString *)description isShowCancel:(BOOL)isShowCancel url:(NSString *)url;
{
    self = [super init];
    if (self) {
...
- (void)_setupUI
{
    ...
    UIImageView *updateIcon = [[UIImageView alloc]initWithFrame:CGRectMake((updateView.frame.size.width - Ratio(100))/2, Ratio(30), Ratio(100), Ratio(100))];
...
//    versionLabel.text = [NSString stringWithFormat:@"发现新版本 V%@",self.version];
    ...
    //更新内容
    UITextView *descTextView = [[UITextView alloc]initWithFrame:CGRectMake(Ratio(28), Ratio(30) + CGRectGetMaxY(versionLabel.frame), updateView.frame.size.width - Ratio(56), descHeight)];
    ...
    descTextView.textColor = [UIColor colorFromHexString:@"AAAAAA"];
    ...
    updateButton.backgroundColor = [UIColor colorFromHexString:@"FDAB05"];
    updateButton.frame = CGRectMake(Ratio(25), updateView.bottom- Ratio(90), updateView.frame.size.width - Ratio(50), Ratio(40));
    updateButton.clipsToBounds = YES;
    updateButton.layer.cornerRadius = Ratio(40/2);
    ...
    cancelButton.center = CGPointMake(CGRectGetMaxX(updateView.frame)-Ratio(40/2), CGRectGetMinY(updateView.frame)+Ratio(40/2));
    cancelButton.bounds = CGRectMake(0, 0, Ratio(40), Ratio(40));
    [cancelButton setBackgroundImage:[[UIImage imageNamed:@"VersionUpdate_Cancel"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
