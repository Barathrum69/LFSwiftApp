...
-(void)initView
{
    ...
    //    _codeTextField.textColor = [UIColor colorFromHexString:@"222222"];
    ...
    _codeTextField.textFieldDidChange = ^(NSString * _Nonnull text) {
        if (text.length > 6) {
            weakself.codeTextField.text = [text substringToIndex:6];
        }
        [weakself noticationNextBtn];
    };
...
-(void)nextBtnOnClick
{
    ...
    [LoginRegisterProxy modifyPhoneNumerWithPhoneNum:_phoneTextField.text phoneCountryCode:_countryCodeString code:_codeTextField.text success:^(BOOL isSuccess) {
        [self showToast:@"修改成功"];
...
-(void)codeBtnOnClick
{
    ...
    //验证码点击
    [LoginRegisterProxy regirstLoginVerificationCodeWithPhoneNum:_phoneTextField.text phoneCountryCode:_countryCodeString codeType:@"6" success:^(BOOL isSuccess, int code) {
        [STTextHudTool hideSTHud];
        [self showToast:@"已发送"];
        [self startTime];
    } failure:^(NSError * _Nonnull error) {
