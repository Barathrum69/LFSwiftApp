...
-(void)initView
{
    ...
    UILabel *titleLab = [[UILabel alloc]initWithFrame:CGRectMake(0, kWidth(128), kScreenWidth, kWidth(19))];
    titleLab.textColor = [UIColor whiteColor];
    titleLab.text = @"忘记密码";
    titleLab.font = [UIFont systemFontOfSize:kWidth(19)];
...
//    [_countryCodeBtn addTarget:self action:@selector(btnOnClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    _phoneTextField = [[LeftRightTextField alloc]initWithFrame:CGRectMake(kWidth(40),titleLab.bottom+kWidth(44) , kScreenWidth-kWidth(40*2), kWidth(45))];
...
//    _phoneTextField.textAlignment = NSTextAlignmentCenter;
    ...
    _codeTextField =  [[LeftRightTextField alloc]initWithFrame:CGRectMake(kWidth(40),_phoneTextField.bottom+kWidth(15) , kScreenWidth-kWidth(40*2), kWidth(45))];
    ...
    NSAttributedString *attrString2 = [[NSAttributedString alloc] initWithString:@"请输入验证码" attributes:
        @{NSForegroundColorAttributeName:[UIColor colorFromHexString:@"CCCCCC"],
                     NSFontAttributeName:_codeTextField.font}];
...
//    [_passWordTextField addSubview:self.pswBtn];
    _passWordTextField =  [[LeftRightTextField alloc]initWithFrame:CGRectMake(kWidth(40),_codeTextField.bottom+kWidth(15) , kScreenWidth-kWidth(40*2), kWidth(45))];
    ...
    NSAttributedString *attrString1 = [[NSAttributedString alloc] initWithString:@"请输入新密码(6-10位字母和数字)" attributes:
        @{NSForegroundColorAttributeName:[UIColor colorFromHexString:@"CCCCCC"],
    ...
    _pswBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, kWidth(45-30)/2, kWidth(30), kWidth(30))];
    ...
    //注册按钮
    _registerBtn = [[UIButton alloc]initWithFrame:CGRectMake(_passWordTextField.left, _passWordTextField.bottom+kWidth(21), _passWordTextField.width, kWidth(42))];
...
//            weakself.codeBtn.backgroundColor = [UIColor colorFromHexString:@"F67C37" withAlph:0.5];
...
-(void)registerBtnOnClick
{
    ...
    if (self.passWordTextField.text.length >11 ||self.passWordTextField.text.length < 6) {
        [self showToast:@"密码格式错误(密码为6-10位字母和数字)"];
        return;
    }
    ...
    [self loadFindPswRequest];
    
}//注册按钮点击事件

-(void)loadFindPswRequest
{
    ...
    [LoginRegisterProxy registerSaltSuccess:^(NSString * _Nonnull salt) {
        ...
        [LoginRegisterProxy findPswWithPhoneNum:self.phoneTextField.text code:self.codeTextField.text userPass:string phoneCountryCode:self.countryCodeString success:^(BOOL isSuccess) {
...
- (void)codeBtnOnClick
{
    ...
    [STTextHudTool showWaitText:@"请求中..."];
    [LoginRegisterProxy regirstLoginVerificationCodeWithPhoneNum:self.phoneTextField.text phoneCountryCode:self.countryCodeString codeType:@"3" success:^(BOOL isSuccess,int code) {
        [STTextHudTool hideSTHud];
        if (isSuccess) {
            [self startTime];
            [self showToast:@"已发送"];
